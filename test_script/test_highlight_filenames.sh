...
test_failure_int()
{
  ...
  assert_stdout_includes "ERROR: highlight_filenames must be an Array of Strings"
  assert_stdout_includes "--languages ${tmp_url}"
  assert_stdout_includes "manifest='languages-csharp-nunit/start_point/manifest.json'"
  assert_stdout_includes '"highlight_filenames": 42'
...
test_failure_empty_array()
{
  ...
  assert_stdout_includes "--languages ${tmp_url}"
  assert_stdout_includes "manifest='languages-csharp-nunit/start_point/manifest.json'"
  assert_stdout_includes '"highlight_filenames": []'
  # assert_stderr_line_count_equals 4
  assert_status_equals "${error_code}"
}
...
test_failure_not_visible()
{
  ...
  refute_image_created
  assert_stdout_includes "ERROR: highlight_filenames[1]=\"xx.cs\" not in visible_filenames"
  assert_stdout_includes "--languages ${tmp_url}"
  assert_stdout_includes "manifest='languages-csharp-nunit/start_point/manifest.json'"
  assert_stdout_includes '"highlight_filenames": ["Hiker.cs", "xx.cs"]'
  # assert_stderr_line_count_equals 4
  assert_status_equals "${error_code}"
}
...
test_failure_duplicates()
{
  ...
  refute_image_created
  assert_stdout_includes "ERROR: highlight_filenames[0, 2] are duplicates of \"Hiker.cs\""
  assert_stdout_includes "--languages ${tmp_url}"
  assert_stdout_includes "manifest='languages-csharp-nunit/start_point/manifest.json'"
  assert_stdout_includes '"highlight_filenames": ["Hiker.cs", "HikerTest.cs", "Hiker.cs"]'
  # assert_stderr_line_count_equals 4
  assert_status_equals "${error_code}"
}
