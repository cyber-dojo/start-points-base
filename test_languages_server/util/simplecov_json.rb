...
# A SimpleCov formatter writing coverage.json, which holds the per-group line
# and branch totals that check_test_results.rb reads. SimpleCov ships its own
# JSON formatter, shaped per file rather than per group, so this one carries
# its own name rather than reopening that class and redefining its format
# method. Redefining it makes ruby -w report the redefinition, and this suite
# counts warnings.
#
# based on https://github.com/vicentllongo/simplecov-json
class CoverageGroupsFormatter
  def format(result)
    groups = {}
