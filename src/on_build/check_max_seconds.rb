...
module CheckMaxSeconds
  ...
  def check_max_seconds(url, manifest_filename, json, error_code)
    if json.has_key?('max_seconds')
      ...
      exit_unless_max_seconds_is_integer(max_seconds, url, manifest_filename, json, error_code)
      exit_unless_max_seconds_in_range(max_seconds, url, manifest_filename, json, error_code)
    end
  end

  # - - - - - - - - - - - - - - - - - - - - - - -

  def exit_unless_max_seconds_is_integer(max_seconds, url, manifest_filename, json, error_code)
  ...
  def exit_unless_max_seconds_in_range(max_seconds, url, manifest_filename, json, error_code)
