...
case image_name
when '--custom'    then ba_error(4, '--custom requires preceding <image_name>')
when '--exercises' then ba_error(5, '--exercises requires preceding <image_name>')
when '--languages' then ba_error(6, '--languages requires preceding <image_name>')
end

unless ['--custom','--exercises','--languages'].include?(image_type)
  ba_error(7, "<image-name> must be followed by one of --custom/--exercises/--languages")
end

if git_repo_urls == []
  case image_type
  when '--custom'    then ba_error(8, '--custom requires at least one <git-repo-url>')
  when '--exercises' then ba_error(9, '--exercises requires at least one <git-repo-url>')
  when '--languages' then ba_error(10, '--languages requires at least one <git-repo-url>')
  end
end
...
unless well_formed_image_name?(image_name)
  ba_error(14, "malformed <image-name> #{image_name}")
end
