...
def get_existing_image_names
  # images already on the node
  `docker image ls --format "{{.Repository}}:{{.Tag}}"`.split("\n").sort.uniq
end
...
def pull_images_not_on_node
  current_image_names = get_current_image_names
  existing_image_names = get_existing_image_names
